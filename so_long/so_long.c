...
/*   Updated: 2024/04/02 20:05:43 by tlupu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  flood_fill(char **tab, t_mlx size, t_mlx begin, int *collectibles, int *exits)
{
    char c;
    t_mlx p;
...
int 	check_path(t_mlx *win)
{
    ...
    begin.x = start_y;
    begin.y = start_x; 
	 char **map_copy = malloc(sizeof(char *) * (countrow + 1));
...
int	display_next_frame_and_door(t_mlx *win)
{
	...
	while (win->arr[i] != NULL)
	{
		...
		while (win->arr[i][j] != '\0')
		{
			if (win->arr[i][j] == 'P' && win->exit == true && ((i > 0
						&& win->arr[i - 1][j] == 'E') || (i < win->height - 1
						&& win->arr[i + 1][j] == 'E') || (j > 0 && win->arr[i][j
						- 1] == 'E') || (j < win->width - 1 && win->arr[i][j
						+ 1] == 'E')))
			{
	...
	void update_position(t_mlx * win, int key_code, char **arr)
	{
		...
		img_height = 64;
		img_width = 64;
		win->character_img_l = mlx_xpm_file_to_image(win->mlx, path_ml,
				&img_width, &img_height);
		win->character_img_r = mlx_xpm_file_to_image(win->mlx, path_r,
				&img_width, &img_height);
		win->character_img_ur = mlx_xpm_file_to_image(win->mlx, path_C,
				&img_width, &img_height);
		while (arr[i] != NULL)
		{
			j = 0;
	...
	int main(int argc, char **argv)
	{
		...
		img_height = 64;
		win.arr = arr;
		int num = check_path(&win);
		if (validate == 0)
		{
			i = 0;
			win.mlx = mlx_init();
			win.mlx_win = mlx_new_window(win.mlx, 64 * win.width, 64
					* win.height, "So_Long");
